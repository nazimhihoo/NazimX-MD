   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +92346320**")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 💀\nFor example: +92346320**: `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            ...
            console.log(chalk.magenta(`${themeemoji} GITHUB: https://github.com/nazimhihoo `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @none `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER:+923463206754${owner}`))
